    ...
    , ui(new Ui::InfoWindow)
{
    ...
    (globals().show_content_info_video)
        ? ui->notShow->setChecked(false)
        : ui->notShow->setChecked(true);

    ui->textBrowser->setFocus();

    connect(this, &InfoWindow::typeInfoChanged,
            this, &InfoWindow::slot_typeInfoChanged, Qt::UniqueConnection);
...
void InfoWindow::setTex(const QString content)
{
    ...
    } else if (m_typeInfo == INFO_VIDEO ||
               m_typeInfo == INFO_REPORT)
    {
...
void InfoWindow::slot_typeInfoChanged()
{
    if (m_typeInfo == INFO_REALEASE) {
        ...
        ui->label_image->setPixmap(QPixmap(QString::fromUtf8(":/img/documents/history.png")));
