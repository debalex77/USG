...
{
    const QString threadConnectionName = QString("connection_%1")
...
void HandlerFunctionThread::syncWithMySQL(int localPatientId)
{
    ...
    if (! dbCloud.open()) {
        qWarning(logWarning()) << "[SYNC] Conectarea la MySQL (cloud) a eșuat: "
                               << dbCloud.lastError().text();
        return;
    }
    ...
    qrySelect.prepare("SELECT * FROM patients WHERE id = ?");
    ...
    qryIDNP_cloud.prepare(R"(
        SELECT 1 FROM patients WHERE idnp = ? LIMIT 1
    )");
    ...
    qryName_cloud.prepare(R"(
        ...
        WHERE last_name = ? AND first_name = ? AND birthday = ? LIMIT 1
    )");
    ...
    qryCloud.prepare(R"(
        INSERT INTO patients (
            deletion_mark, idnp, last_name, first_name, middle_name,
            medical_policy, birthday, address, telephone, email, comment)
        VALUES (:deletion_mark, :idnp, :last_name, :first_name, :middle_name,
            :medical_policy, :birthday, :address, :telephone, :email, :comment)
        ON DUPLICATE KEY UPDATE
            deletion_mark = VALUES(deletion_mark), idnp = VALUES(idnp), last_name = VALUES(last_name), first_name = VALUES(first_name), middle_name = VALUES(middle_name),
            medical_policy = VALUES(medical_policy), birthday = VALUES(birthday), address = VALUES(address), telephone = VALUES(telephone),
            email = VALUES(email), comment = VALUES(comment)
    )");
    // id nu adaugam
    qryCloud.bindValue(":deletion_mark", qrySelect.value("deletion_mark"));
    qryCloud.bindValue(":idnp", qrySelect.value("idnp"));
    qryCloud.bindValue(":last_name", qrySelect.value("last_name"));
    qryCloud.bindValue(":first_name", qrySelect.value("first_name"));
    qryCloud.bindValue(":middle_name", qrySelect.value("middle_name"));
    qryCloud.bindValue(":medical_policy", qrySelect.value("medical_policy"));
...
void HandlerFunctionThread::updateDataPatientInDB()
{
    ...
    { // -- bloc pentru dbConnection
        ...
        { // -- bloc pentru QSqlQuery
            QSqlQuery qry(dbConnection);
            qry.prepare(db->getTextSQL(":/sql/queries/patients_update.sql"));
            qry.addBindValue(0);
...
void HandlerFunctionThread::exportDocumentsToPDF()
{
    ...
    const QString threadConnectionName = QString("connection_%1")
                                             .arg(reinterpret_cast<quintptr>(QThread::currentThreadId()));
    const QString threadImageConnectionName = QString("connection_image_%1")
                                                  .arg(reinterpret_cast<quintptr>(QThread::currentThreadId()));
...
{
    QSqlQuery qry(dbConnection);
    qry.prepare(db->getTextSQL(":/sql/queries/patients_insert.sql"));
    qry.addBindValue(m_DataCatPatient.id);
    qry.addBindValue(0);
    qry.addBindValue((m_DataCatPatient.idnp.isEmpty()) ? QVariant() : m_DataCatPatient.idnp);
    qry.addBindValue(m_DataCatPatient.name);
...

    {
        ...
        if (! qry.exec()) {
            ...
            while (qry.next()) {
                ...
                m_DocumentExportEmailData.id_patient = qry.value(rec.indexOf("patient_id")).toInt();
                ...
                data.name_patient = QStringLiteral("%1 %2")
...

    {
        ...
        QSqlQuery qry(dbConnection);
        {
            ...
            if (m_TagsSystemDocument.gestation2){
                ...
                const QString pathToFileGestation = m_pathTemplatesDocs + "/Gestation2.lrxml";
