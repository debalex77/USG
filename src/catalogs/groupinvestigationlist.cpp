...
GroupInvestigationList::GroupInvestigationList(DataBase &db, QWidget *parent)
    : QDialog(parent)
    ...
    , m_db(db)
{
    ...
    setWindowIcon(QIcon(":/img/common/folder_yellow.png"));
...
void GroupInvestigationList::updateTable()
{
    ...
    qry.prepare(R"(
        ...
        FROM
            investigations i
        INNER JOIN
            investigationsGroup g ON g.id = i.owner
        WHERE
            i.owner IS NOT NULL
        GROUP BY
            i.owner, g.name, g.cod
        ORDER BY
            g.cod ASC
    )");
    ...
    if (qry.exec()){
        while (qry.next()) {
            ...
            result = result + "gr_" + qry.value(1).toString() + "\n";
            ...
            qry_items.bindValue(":owner", qry.value(0));
            if (qry_items.exec()){
                while (qry_items.next()) {
                    result += " it_" + qry_items.value(0).toString() + " - " + qry_items.value(1).toString() + "\n";
                }
            }
        }
    }
...
void GroupInvestigationList::onPrint()
{
    ...
    m_investigations = new QSqlQuery(R"(
        SELECT
            cod,
            name
        FROM
    ...
    m_report->loadFromFile(dir.toNativeSeparators(globals().docsTemplatesPath + "/Tree_investigations.lrxml"));
