...
Reports::Reports(DataBase &db, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Reports),
...
void Reports::initConnections()
{
    ...
    connect(m_report, &LimeReport::ReportEngine::renderStarted,
            this, &Reports::renderStarted);
    connect(m_report, QOverload<int>::of(&LimeReport::ReportEngine::renderPageFinished),
            this, QOverload<int>::of(&Reports::renderPageFinished));
    connect(m_report, &LimeReport::ReportEngine::renderFinished,
            this, &Reports::renderFinished);
    ...
    connect(ui->btnZoomIn, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::zoomIn);
    connect(ui->btnZoomOut, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::zoomOut);
    ...
    connect(ui->btnBackPage, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::priorPage);
    connect(ui->btnNextPage, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::nextPage);
    ...
    connect(ui->btnPrintReport, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::print);
    connect(ui->btnExportPdf, &QToolButton::clicked,
            m_preview, &LimeReport::PreviewReportWidget::printToPDF);

    // filter
    connect(ui->btnChoicePeriod, &QToolButton::clicked,
            this, &Reports::openCustomPeriod);
    connect(ui->comboTypeReport, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, QOverload<int>::of(&Reports::typeReportsCurrentIndexChanged));
    ...
    connect(ui->btnEditOrganization, &QToolButton::clicked,
            this, &Reports::openCatOrganization);
    connect(ui->btnEditContract, &QToolButton::clicked,
            this, &Reports::openCatContract);
    connect(ui->btnEditDoctor, &QToolButton::clicked,
            this, &Reports::openCatDoctor);

    connect(ui->btnGenerateReport, &QToolButton::clicked,
            this, &Reports::generateReport);
    connect(ui->btnOpenDesigner, &QToolButton::clicked,
            this, &Reports::openDesignerReport);
    connect(ui->btnSettingsReport, &QToolButton::clicked,
            this, &Reports::openSettingsReport);
    connect(ui->btnSendEmail, &QToolButton::clicked,
            this, &Reports::sendReportToEmail);
}

void Reports::initDisconnections()
{
    // butoane principale ale generatorului de rapoarte
    disconnect(m_report, &LimeReport::ReportEngine::renderStarted,
               this, &Reports::renderStarted);
    disconnect(m_report, QOverload<int>::of(&LimeReport::ReportEngine::renderPageFinished),
               this, QOverload<int>::of(&Reports::renderPageFinished));
    disconnect(m_report, &LimeReport::ReportEngine::renderFinished,
               this, &Reports::renderFinished);

    disconnect(ui->comboScalePercent, QOverload<int>::of(&QComboBox::currentIndexChanged),
               this, QOverload<int>::of(&Reports::slotScalePercentChanged));
    disconnect(ui->btnZoomIn, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::zoomIn);
    disconnect(ui->btnZoomOut, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::zoomOut);
    ...
    disconnect(ui->pageNavigator, QOverload<int>::of(&QSpinBox::valueChanged),
               this, QOverload<int>::of(&Reports::slotPageNavigatorChanged));
    disconnect(ui->btnBackPage, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::priorPage);
    disconnect(ui->btnNextPage, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::nextPage);
    ...
    disconnect(ui->btnPrintReport, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::print);
    disconnect(ui->btnExportPdf, &QToolButton::clicked,
               m_preview, &LimeReport::PreviewReportWidget::printToPDF);

    // filter
    disconnect(ui->btnChoicePeriod, &QToolButton::clicked,
               this, &Reports::openCustomPeriod);
    disconnect(ui->comboTypeReport, QOverload<int>::of(&QComboBox::currentIndexChanged),
               this, QOverload<int>::of(&Reports::typeReportsCurrentIndexChanged));
    ...
    disconnect(ui->btnEditOrganization, &QToolButton::clicked,
               this, &Reports::openCatOrganization);
    disconnect(ui->btnEditContract, &QToolButton::clicked,
               this, &Reports::openCatContract);
    disconnect(ui->btnEditDoctor, &QToolButton::clicked,
               this, &Reports::openCatDoctor);

    disconnect(ui->btnGenerateReport, &QToolButton::clicked,
               this, &Reports::generateReport);
    disconnect(ui->btnOpenDesigner, &QToolButton::clicked,
               this, &Reports::openDesignerReport);
    disconnect(ui->btnSettingsReport, &QToolButton::clicked,
               this, &Reports::openSettingsReport);
    disconnect(ui->btnSendEmail, &QToolButton::clicked,
               this, &Reports::sendReportToEmail);
}
    ...
    if (name_report == "Lista pacientilor (filtru - organizatii)"){
        ...
        if (globals().thisMySQL)
            str_qry = "SELECT "
                      ...
                      "  CONCAT(patients.last_name ,' ', patients.first_name ,', ', DATE_FORMAT(patients.birthday, '%d.%m.%Y')) AS Pacients,"
                      "  patients.idnp,"
            ...
            str_qry = "SELECT "
                      ...
                      "  patients.last_name || ' ' || patients.first_name || ', ' || strftime('%d.%m.%Y', patients.birthday) AS Pacients,"
                      "  patients.idnp,"
        ...
        str_qry = str_qry + "   organizations.name AS organization"
                            "  FROM orderEcho "
                            "  INNER JOIN patients ON orderEcho.patient_id = patients.id"
        ...
        return QString("SELECT orderEcho.id, "
                       ...
                       "  patients.last_name || ' ' || patients.first_name || ', ' || substr(patients.birthday, 9, 2) ||'.'|| substr(patients.birthday, 6, 2) ||'.'|| substr(patients.birthday, 1, 4)  AS Pacients, "
...
void Reports::generateReport()
{
    ...
    print_model_organization->setQuery(m_db.getQryFromTableConstantById(globals().idUserApp));
...
void Reports::organizationCurrentIndexChanged(const int index)
{
    ...
    if (m_db.getObjectDataById("organizations", id_organization, items)){
...
void Reports::openCatOrganization()
{
    ...
    organization->setProperty("id", id_organization);
    organization->show();
}
...
void Reports::openCatContract()
{
    ...
    contract->setProperty("ItNew", false);
    contract->setProperty("Id", id_contract);
    contract->setProperty("IdOrganization", id_organization);
    contract->show();
}
