...
QWidget *MdiAreaContainer::findWidgetByKey(const QString &key) const
{
    updateCachedSubWindowList();

    for (QMdiSubWindow *subWindow : std::as_const(cachedSubWindowList)) {
        if (!subWindow)
            continue;
...
bool MdiAreaContainer::activateIfExists(const QString &key)
{
    updateCachedSubWindowList();

    QMdiSubWindow *target = nullptr;
    QMdiSubWindow *oldActive = m_mdiArea->activeSubWindow();
