...
void DocEmailExporterWorker::exportOrderEcho(QSqlDatabase &dbConn)
{
    ...
    // Modelele sunt copii ale workerului si sunt eliberate de acesta.
    // LimeReport nu trebuie sa preia proprietatea lor.
    ...
    if (m_report->printToPDF(m_data.filePDF + "/Comanda_ecografica_nr_"
...
void DocEmailExporterWorker::exportReportEcho(QSqlDatabase &dbConn)
{
    ...
    m_report->dataManager()->addModel("table_logo", model_img, false);
    m_report->dataManager()->addModel("main_organization", model_organization, false);
    ...
    if (organs_internal && urinary_system){
        ...
        // export documentului
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_complex.pdf")) {
    ...
    if (organs_internal && ! urinary_system){
        ...
        // export documentului
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_organe_interne.pdf")) {
    ...
    if (prostate){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_prostata.pdf")) {
    ...
    if (gynecology){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_ginecologie.pdf")) {
    ...
    if (breast){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_glandele_mamare.pdf")) {
    ...
    if (thyroide){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_tiroida.pdf")) {
    ...
    if (gestation0){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_sarcina0.pdf")) {
    ...
    if (gestation1){
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_sarcina1.pdf")) {
    ...
    if (gestation2){
        ...
        const int trim = modelGestation2->record(0).value("trimestru").toInt();
        const QString pathToFileGestation = m_data.pathTemplatesDocs + "/Gestation2.lrxml";
        ...
        m_report->setShowProgressDialog(true);
        if (m_report->printToPDF(m_data.filePDF + "/Raport_ecografic_nr_" + fileSafeDocumentNumber(m_data.nr_report) + "_sarcina2.pdf")) {
        ...
        if (qry.next()){
            ...
            for (int i = 0; i < imageByteArrays.size(); ++i) {
                if (! imageByteArrays[i].isEmpty()) {
                    QImage image;
                    if (image.loadFromData(imageByteArrays[i])) {
