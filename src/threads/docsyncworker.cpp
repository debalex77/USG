...
{
    QSqlQuery qry_local(dbConn_local);
    qry_local.prepare(R"(SELECT * FROM patients WHERE id = ?)");
    qry_local.addBindValue(m_data.id_patient);
    if (qry_local.exec() && qry_local.next()) {
        QSqlRecord rec = qry_local.record();
        m_data.name_parient     = qry_local.value(rec.indexOf("last_name")).toString();
        m_data.fname_patient    = qry_local.value(rec.indexOf("first_name")).toString();
...
bool docSyncWorker::existreportDoc(QSqlDatabase &dbConn_sync)
{
    ...
    qry.prepare(R"(
        SELECT
            ...
            reportEcho.patient_id  AS id_patient,
