SELECT
    ...
    CONCAT_WS(' ', p.last_name, NULLIF(p.first_name, '')) AS pacientFullName,
    p.idnp AS pacientIDNP,
    CONCAT(dt.name, ' ', LEFT(dt.fName, 1), '.') AS doctorName,
    u.name AS userName,
    CONCAT(
        CONCAT_WS(' ', p.last_name, NULLIF(p.first_name, '')),
        ', idnp: ', COALESCE(p.idnp, ''),
