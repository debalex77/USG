    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            ...
            if (qry.value(rec.indexOf("t_organs_internal")).toBool())
                syncOrgansInternal(dbConn_sync, dbConn_local);

            // s.urinar
            if (qry.value(rec.indexOf("t_urinary_system")).toBool())
                syncUrinarySystem(dbConn_sync, dbConn_local);

            // prostata
            if (qry.value(rec.indexOf("t_prostate")).toBool())
                syncProstate(dbConn_sync, dbConn_local);

            // ginecologia
            if (qry.value(rec.indexOf("t_gynecology")).toBool())
                syncGynecology(dbConn_sync, dbConn_local);

            // gl.mamare
            if (qry.value(rec.indexOf("t_breast")).toBool())
                syncBreast(dbConn_sync, dbConn_local);

            // gl.tiroida
            if (qry.value(rec.indexOf("t_thyroid")).toBool())
                syncThyroid(dbConn_sync, dbConn_local);

            // gestation0
            if (qry.value(rec.indexOf("t_gestation0")).toBool())
                syncGestation0(dbConn_sync, dbConn_local);

            // gestation1
            if (qry.value(rec.indexOf("t_gestation1")).toBool())
                syncGestation1(dbConn_sync, dbConn_local);

            // gestation2
            if (qry.value(rec.indexOf("t_gestation2")).toBool())
                syncGestation2(dbConn_sync, dbConn_local);
        }
    }
...
void docSyncWorker::syncOrgansInternal(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableLiver:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableLiver' a documentului nr."
                             << m_datesSync.nr_report;
        }
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableCholecist:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableCholecist' a documentului nr."
                             << m_datesSync.nr_report;
        }
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tablePancreas:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tablePancreas' a documentului nr."
                             << m_datesSync.nr_report;
        }
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableSpleen:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableSpleen' a documentului nr."
                             << m_datesSync.nr_report;
        }
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableIntestinalLoop:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableIntestinalLoop' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncUrinarySystem(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableKidney:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableKidney' a documentului nr."
                             << m_datesSync.nr_report;
        }
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableBladder:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableBladder' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncProstate(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableProstate:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableProstate' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncGynecology(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableGynecology:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableGynecology' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncBreast(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        qry_sync.addBindValue(qry.value(rec.indexOf("recommendation")));
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableBreast:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableBreast' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncThyroid(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        qry_sync.addBindValue(qry.value(rec.indexOf("recommendation")));
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableThyroid:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableThyroid' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncGestation0(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableGestation0:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableGestation0' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncGestation1(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
    ...
    if (qry.exec() && qry.next()) {
        ...
        qry_sync.addBindValue(qry.value(rec.indexOf("lmp")));
        if (! qry_sync.exec()) {
            qCritical(logCritical()) << "[SYNC] Eroare la sincronizarea tabelei tableGestation1:"
                                     << qry_sync.lastError().text();
            dbConn_sync.rollback();
            return;
        } else {
            qInfo(logInfo()) << "[SYNC] Sincronizarea cu succes a tabelei 'tableGestation1' a documentului nr."
                             << m_datesSync.nr_report;
        }
...
void docSyncWorker::syncGestation2(QSqlDatabase &dbConn_sync, QSqlDatabase &dbConn_local)
{
    QSqlQuery qry(dbConn_local);
    QSqlQuery qry_sync(dbConn_sync);
